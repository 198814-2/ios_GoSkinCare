...
//#define kMenuTransformScale CATransform3DMakeScale(0.9f, 0.9f, 0.9f)
#define kMenuTransformScale CATransform3DMakeScale(1.f, 1.f, 1.f)
...
typedef enum {
    SlidePanningStateStopped,
    SlidePanningStateLeft,
    SlidePanningStateRight
} SlidePanningState;

@interface SlideMenuMainViewController ()<UIGestureRecognizerDelegate>
{
    SlidePanningState panningState;
...
@property (strong, nonatomic) SlideMenuLeftMenuSegue *leftSegue;
...
+ (SlideMenuMainViewController *)getInstanceForVC:(UIViewController *)vc {
    ...
    for (SlideMenuMainViewController *mainVC in allInstances) {
...
- (void)setup {
    ...
    if ([self primaryMenu] == PrimaryMenuLeft) {
        @try
        {
            [self performSegueWithIdentifier:Segue_ShowLeftMenu sender:self];
    ...
    else if ([self primaryMenu] == PrimaryMenuRight) {
        @try
        {
            @try {
                [self performSegueWithIdentifier:Segue_ShowLeftMenu sender:self];
    ...
    if ([self primaryMenu] == PrimaryMenuLeft) {
        if (self.leftMenu) {
            self.leftSegue = [[SlideMenuLeftMenuSegue alloc] initWithIdentifier:@"leftMenu" source:self destination:self.leftMenu];
        ...
        if (self.rightMenu) {
            self.rightSegue = [[SlideMenuRightMenuSegue alloc] initWithIdentifier:@"rightSegue" source:self destination:self.rightMenu];
...
- (void)openLeftMenuAnimated:(BOOL)animated {
    ...
    [UIView animateWithDuration: animated ? self.openAnimationDuration : 0 animations:^{
        ...
        [self addGestures];
        [self enableGestures];
        self.menuState = SlideMenuLeftOpened;
...
- (void)openRightMenuAnimated:(BOOL)animated {
    ...
    [UIView animateWithDuration:animated ? self.openAnimationDuration : 0 animations:^{
        ...
        [self addGestures];
        [self enableGestures];
        self.menuState = SlideMenuRightOpened;
...
- (void)closeLeftMenuAnimated:(BOOL)animated {
    ...
    [UIView animateWithDuration:animated ? self.closeAnimationDuration : 0 animations:^{
        ...
        self.menuState = SlideMenuClosed;
        [self.currentActiveNVC.view addGestureRecognizer:self.panGesture];
...
- (void)closeMenu {
    if (self.menuState == SlideMenuLeftOpened) {
        [self closeLeftMenu];
    }
    else if (self.menuState == SlideMenuRightOpened) {
        [self closeRightMenu];
    }
}
...
- (void)switchCurrentActiveControllerToController:(UINavigationController *)nvc fromMenu:(UITableViewController *)menu {
    ...
    if (self.isInitialStart) {
        if ([self primaryMenu] == PrimaryMenuLeft) {
            if ([menu isKindOfClass:[SlideMenuLeftTableViewController class]]) {
    ...
    if (![UIApplication sharedApplication].statusBarHidden) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0) {
            if (!UIInterfaceOrientationIsLandscape([UIApplication sharedApplication].statusBarOrientation)) {
                ...
                if (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad || UIDeviceOrientationIsPortrait((UIDeviceOrientation)initialOrientation)) {
                    frame.origin.y = -20;
                }
...
- (void)openContentViewControllerForMenu:(SlideMenu)menu atIndexPath:(NSIndexPath *)indexPath {
    if (menu == SlideMenuLeft) {
        ...
        if ([self respondsToSelector:@selector(navigationControllerForIndexPathInLeftMenu:)]) {
            ...
            SlideMenuContentSegue *segue = [[SlideMenuContentSegue alloc] initWithIdentifier:@"ContentSugue" source:self.leftMenu destination:navController];
            [segue perform];
        }
        else {
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture {
    ...
    if (self.menuState != SlideMenuClosed) {
    ...
    if ([gesture state] == UIGestureRecognizerStateBegan) {
        ...
        if (self.menuState != SlideMenuClosed) {
            ...
            if (panningState == SlidePanningStateRight) {
                if (panningView.frame.origin.x < ([self leftMenuWidth] / 2.0f)) {
                    [self closeLeftMenu];
                }
                else {
            ...
            if (panningState == SlidePanningStateLeft) {
                if (self.view.frame.size.width - (panningView.frame.origin.x + panningView.frame.size.width) < ([self rightMenuWidth] / 2.0f)) {
                    [self closeRightMenu];
                }
                else {
        ...
        panningState = SlidePanningStateStopped;
    }
    else {
        if (!CGPointEqualToPoint(panStartPosition, (CGPoint){0,0})) {
            ...
            if (panStartPosition.x > actualWidth && panStartPosition.x < panningView.frame.size.width - actualWidth && self.menuState == SlideMenuClosed) {
                return;
            }
        }
        ...
        if(panStarted) {
            ...
            if (panningView.frame.origin.x + translation.x < 0) {
                panningState = SlidePanningStateLeft;
                ...
                panningState = SlidePanningStateRight;
        ...
        //----
        if (panningState == SlidePanningStateLeft && self.leftPanDisabled) {
            panningState = SlidePanningStateStopped;
            return;
        }
        else if (panningState == SlidePanningStateRight && self.rightPanDisabled) {
            panningState = SlidePanningStateStopped;
            return;
        }
        //----
        ...
        if (self.menuState == SlideMenuLeftOpened) {
            if (fabs(translation.x) > kPanMinTranslationX && translation.x < 0) {
                ...
                [self configure3DTransformForMenu:SlideMenuLeft panningView:panningView];
            }
        }
        else if (self.menuState == SlideMenuRightOpened) {
            if (fabs(translation.x) > kPanMinTranslationX && translation.x > 0) {
            ...
            else if (self.view.frame.size.width - (panningView.frame.origin.x + panningView.frame.size.width + translation.x) < [self rightMenuWidth] && panningView.frame.origin.x <= 0) {
                [panningView setCenter:CGPointMake([panningView center].x + translation.x, [panningView center].y)];
                
                [self configure3DTransformForMenu:SlideMenuRight panningView:panningView];
            }
        }
        else if (self.menuState == SlideMenuClosed) {
            if (panningState == SlidePanningStateRight && self.leftMenu) {
                if (fabs(translation.x) > kPanMinTranslationX && translation.x > 0) {
                    [self openLeftMenu];
                }
                else if ((panningView.frame.origin.x + translation.x) < [self leftMenuWidth] && (panningView.frame.origin.x + translation.x) > 0) {
...
- (void)configure3DTransformForMenu:(SlideMenu)menu panningView:(UIView *)panningView {
    float cx = 0;
    float cy = 0;
    float cz = 0;
    float opacity = 0;
    ...
    /********************************************* DEEPNESS EFFECT *******************************************************/
    if (menu == SlideMenuLeft && panningView.frame.origin.x != 0 && [self deepnessForLeftMenu]) {
    ...
    /********************************************* STATUS BAR FIX *******************************************************/
    if (menu == SlideMenuLeft && panningView.frame.origin.x != 0) {
        if (self.statusBarView) {
            self.statusBarView.layer.opacity = 1 - panningView.frame.origin.x / [self leftMenuWidth];
        }
    }
    else if (menu == SlideMenuRight && panningView.frame.origin.x != 0) {
        if (self.statusBarView) {
