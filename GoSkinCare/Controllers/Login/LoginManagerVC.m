...
- (void)updateButtonFonts {
    ...
    // JoinNow Button
    NSString* text = @"NOT A GOCONUT? JOIN NOW!";
    ...
    [[APIManager sharedManager] loginWithEmail:email password:password success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (responseDict) {
            ...
            if (success) {
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(loginManager:didSuccessToLoginWithEmail:password:)])
                    [self.delegate loginManager:self didSuccessToLoginWithEmail:email password:password];
...
- (IBAction)tapCancelButton:(UIButton*)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(loginManager:didFailToLoginWithEmail:)])
        [self.delegate loginManager:self didFailToLoginWithEmail:nil];
}
...
- (IBAction)unwindToLoginManagerVC:(UIStoryboardSegue*)segue {
